...
Read at the start of each repair iteration in `/builder` step 4 (per-behavior
...
| Security-review finding (secrets, injection, auth bypass) surfaces during validation | `security-finding` | security-engineer (`/builder`: dispatch, effort high; `/apply-fixes`: annotate only — no Agent/Skill grant) | Never downgrades to inline; unresolved after dispatch → human arbitration. |
...
- `/builder`: classification *is* the diagnosis step orchestrator constraint 7
