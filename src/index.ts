...
const DevTeamPlugin: Plugin = async (context, options) => {
    ...
    event: async ({ event }) => {
      if (event.type === "tui.command.execute") {
        coordinator.handleCommand(
          (event.properties as { command?: string }).command ?? "",
        );
      }
        ...
        if (state.updatesMade) {
          await context.client.tui.showToast({
            body: {
              ...
              message:
                "skills, references, scripts and/or knowledge files have been updated",
              variant: "info",
            },
          });
        }
