...
| --- | --- | --- |
| Project convention or preference | **Project `AGENT.md`** (`.claude/AGENT.md` or repo-root `AGENT.md`) | Loaded every session, applies to all agents |
...
| Agent behavior override for this project | **Project `AGENT.md`** under a `## Agent Overrides` section | Overrides plugin defaults without editing plugin files |
...
- Do not edit files inside the plugin (`~/.opencode/...`). Changes there are overwritten on plugin updates.
- Do not create new agent or skill files in the project. Instead, add override instructions to project `AGENT.md`.

### Project AGENT.md structure for overrides
...
`AGENT.md > Agent Overrides > <agent>` / `REVIEW-CONTEXT.md` entry — is a
