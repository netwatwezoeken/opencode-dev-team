...
Output JSON: per `.opencode/knowledge/review-agent-output-contract.md` (Whole-file load: short, canonical schema).
...
Read `.opencode/knowledge/object-calisthenics.md` before analysis. Whole-file load: the agent needs all nine rules as design-pressure thresholds (especially rule 1 one-indentation-level, rule 2 no-else, rule 7 small-entities) plus the rationale prose tying them to the numeric limits below.
...
After producing findings, run the shared challenger loop in `.opencode/knowledge/adversarial-review-protocol.md` (Whole-file load: the slim shared methodology — The Loop + Output format — read in full), then work these complexity-review-specific challenges:
